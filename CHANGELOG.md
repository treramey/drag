...
- Finish the intent-level tracking transition: standard help, contracts, and
  ...
  hidden internal implementation; document and test the 0.9.x compatibility
  window and 0.10.0 shim removal.
...
- First-class `drag tracking` setup, status, run, review, source, schedule,
  ...
  resumable consolidated draft runs, digest-bound review approval with
  redacted evidence, policy conflicts, approval availability, and stale-set
  invalidation, state migration, versioned compatibility checks, host scheduler
  activation, recoverable setup effects, and a one-release
