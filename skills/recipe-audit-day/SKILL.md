...
2. **Traverse the complete bounded day** Run `drag --output json list "$DATE" --fields worklogs.id,worklogs.issueKey,worklogs.duration,worklogs.interval.startTime,worklogs.interval.endTime,worklogs.description,schedule.dayRequiredDuration,schedule.dayLoggedDuration,pagination.next,pagination.selectedDate`. Follow every `pagination.next` unchanged with `--continue-from`, reusing `pagination.selectedDate`, until the day is complete.
