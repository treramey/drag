...
use tempomat::models::{AddWorklogRequest, ScheduleEntity, WorklogEntity};
use url::Url;
...
impl ApiClient {
    pub fn new(credentials: Credentials, debug: bool) -> Result<Self, CliError> {
        let client = Client::builder()
            .user_agent(concat!("tempomat-rust/", env!("CARGO_PKG_VERSION")))
...
fn api_error(status: StatusCode, body: &[u8]) -> CliError {
    if status == StatusCode::UNAUTHORIZED {
        return CliError::Api(
            "unauthorized; tokens are invalid or expired (run `tempo setup`)".to_owned(),
        );
    }
