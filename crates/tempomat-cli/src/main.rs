...
pub enum CliError {
    #[error(transparent)]
    Core(#[from] tempomat::Error),
    #[error("{0}")]
    InvalidInput(String),
    #[error("tempomat is not configured: {0}")]
...
async fn run(cli: Cli) -> Result<Rendered, CliError> {
    ...
    match cli.command {
        ...
        Command::Completions { shell } => {
            ...
            generate(shell, &mut Cli::command(), "tempo", &mut bytes);
...
fn schema() -> Rendered {
    let data = json!({
        ...
        "environment": ["TEMPOMAT_CONFIG", "TEMPO_TOKEN", "TEMPO_ACCOUNT_ID", "ATLASSIAN_EMAIL", "ATLASSIAN_TOKEN", "ATLASSIAN_HOST"],
        "exitCodes": {"0": "success", "1": "runtime failure", "2": "usage or invalid input"}
    });
    Rendered::new(
        data,
        "Use `tempo --output json schema` for the full CLI contract.".to_owned(),
    )
}
