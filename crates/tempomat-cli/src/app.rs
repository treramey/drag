...
use serde_json::json;
use tempomat::models::{AddWorklogRequest, Worklog, WorklogEntity};
use tempomat::schedule::{create_schedule_details, ScheduleDetails};
use tempomat::time::{
    clock_interval, format_duration, month_bounds, parse_clock, parse_duration_or_interval,
    select_date,
};
use tempomat::tracker::{Tracker, TrackerInterval};
use url::Url;
...
impl App {
    ...
    pub async fn log(&self, args: LogArgs) -> Result<Rendered, CliError> {
        ...
        Ok(Rendered::new(
            ...
            format!(
                "Successfully logged {} to {}, type `tempo d {}` to undo.",
    ...
    fn build_add_request(
        ...
        if parsed.seconds <= 0 {
            return Err(tempomat::Error::NonPositiveDuration.into());
        }
        let start = if let Some(start) = parsed.start_time {
            start
        } else if let Some(start) = &input.value.start {
            parse_clock(start).ok_or_else(|| tempomat::Error::InvalidTime(start.clone()))?
        } else {
