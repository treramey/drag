...
fn public_setup_keeps_scheduler_hooks_and_automatic_submission_separately_authorized(
    ...
    assert_eq!(status["scheduler"]["active"], false);
    assert_eq!(setup["scheduler"]["activationRequired"], true);
...
#[cfg(not(target_os = "windows"))]
#[test]
fn interrupted_setup_records_scheduler_ownership_for_safe_cleanup(
) -> Result<(), Box<dyn std::error::Error>> {
    let directory = tempdir()?;
    let home = directory.path().join("home");
    let data = directory.path().join("state");
    let scheduler = directory.path().join("scheduler");
    let settings = home.join(".claude/settings.json");
    std::fs::create_dir_all(settings.parent().ok_or("settings parent")?)?;
    ...
    tracking()?
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("settings must be a JSON object"));
    let config: Value = serde_json::from_str(&std::fs::read_to_string(data.join("config.json"))?)?;
    assert_eq!(config["installed"], false);
    assert_eq!(
...
#[test]
fn public_setup_rejects_invalid_source_configuration_before_persisting(
) -> Result<(), Box<dyn std::error::Error>> {
    let directory = tempdir()?;
    ...
    tracking()?
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("Git source is invalid"));
    assert!(!data_dir.join("config.json").exists());
    assert!(directory_path.join("not-a-repo").exists());
    Ok(())
}

#[cfg(unix)]
#[test]
fn source_identity_rejects_symlink_aliases_of_the_same_repository(
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracking()?
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("duplicate selection"));
...
#[test]
fn setup_reruns_preserve_omitted_sources_and_uninstall_disables_public_runs(
) -> Result<(), Box<dyn std::error::Error>> {
    let directory = tempdir()?;
    let data_dir = directory.path().join("state");
    let repo = directory.path().join("repo");
    ...
    let persisted: Value =
        serde_json::from_str(&std::fs::read_to_string(data_dir.join("config.json"))?)?;
    assert_eq!(persisted["sources"].as_array().map(Vec::len), Some(1));
    assert_eq!(persisted["submission"]["mode"], "review");
    json_output(
        tracking()?
            .args(["--data-dir", data_dir.to_string_lossy().as_ref()])
    ...
    let persisted: Value =
        serde_json::from_str(&std::fs::read_to_string(data_dir.join("config.json"))?)?;
    assert_eq!(persisted["submission"]["mode"], "draft");
    assert_eq!(
        persisted["submission"]["automaticSubmissionAuthorized"],
        false
    );
    Ok(())
}
...
#[test]
fn collection_rechecks_calendar_size_after_configuration() -> Result<(), Box<dyn std::error::Error>>
{
    let directory = tempdir()?;
    let data_dir = directory.path().join("state");
    ...
    json_output(
        tracking()?
            .args(["--data-dir", data_dir.to_string_lossy().as_ref()])
            .args(["setup", "--ics", calendar.to_string_lossy().as_ref()]),
    )?;
    std::fs::write(
        &calendar,
        format!(
    ...
    assert_eq!(run["status"], "source-failed");
    assert!(run["collectionFailures"][0]["error"]
        .as_str()
...
fn public_schedule_aliases_and_run_keep_one_intent_level_result(
    ...
    tracking()?
        ...
        .failure()
        .stderr(predicate::str::contains(
            "host scheduler activation was not completed",
        ));
...
fn public_run_persists_an_actionable_failure_and_resumes_after_an_incomplete_drag_read(
    ...
    conn.execute(
        "INSERT INTO mutation_operations (id, state, idempotency_key, local_date, tempo_account, payload_json, submitting_intent_json, policy_schema_version, payload_schema_version) VALUES ('uncertain-public-run', 'uncertain', 'uncertain-public-run', '2026-03-08', 'account-default', '{}', '{}', 1, 1)",
        [],
    )?;
    drop(conn);
    ...
    assert_eq!(failure["error"]["details"]["uncertain"], true);
    assert_eq!(
...
#[test]
fn public_run_generates_proposals_from_the_configured_provider_stage(
) -> Result<(), Box<dyn std::error::Error>> {
    let directory = tempdir()?;
    let data_dir = directory.path().join("state");
    seed_proposal_bundle(data_dir.to_str().ok_or("data dir")?)?;
    let fixture = write_provider_fixture(&directory, "public.json", valid_provider_response())?;
    json_output(
        tracking()?
            .args(["--data-dir", data_dir.to_string_lossy().as_ref()])
            .args([
...
#[test]
fn public_run_gates_audit_when_an_enabled_source_fails() -> Result<(), Box<dyn std::error::Error>> {
    let directory = tempdir()?;
    let data_dir = directory.path().join("state");
    let repo = directory.path().join("repo");
    std::fs::create_dir(&repo)?;
...
#[test]
fn retention_preserves_raw_evidence_for_pending_proposals() -> Result<(), Box<dyn std::error::Error>>
{
    let directory = tempdir()?;
    let data_dir = directory.path().join("state");
    seed_proposal_bundle(data_dir.to_str().ok_or("data dir")?)?;
    ...
    let journal_event = serde_json::json!({
        "schemaVersion": 1,
        "eventId": "evidence.git.abc123",
        "eventType": "evidence.captured",
        "observedAt": "2026-03-08T09:00:00-04:00",
        "source": {"kind": "fixture", "adapter": "fixture", "reference": "repo#abc123"},
        "collector": {"name": "fixture", "version": "test"},
        "timestampSemantics": {"observedAtSource": "fixture", "timezone": "America/New_York", "explicitDate": "2026-03-08"},
        "privacy": {"classification": "local-fixture", "redacted": false},
        "retention": {"policy": "age-based", "retainUntil": null},
        "supersedes": null,
        "payload": {"summary": "Implemented proposal adapter"},
        "integrityHash": "sha256:test"
    });
    std::fs::write(data_dir.join("journal.jsonl"), format!("{journal_event}\n"))?;
    ...
    let result = json_output(
        companion()?
            ...
            .env("DRAG_COMPANION_RETENTION_NOW", "2027-03-08T00:00:00Z")
            .env("DRAG_TRACKING_RETENTION_RAW_DAYS", "1"),
    )?;
    assert!(result["classes"]["raw"]["protected"].as_u64().unwrap_or(0) > 0);
    assert_eq!(result["classes"]["raw"]["expired"], 0);
    let journal = std::fs::read_to_string(data_dir.join("journal.jsonl"))?;
    assert!(journal.contains("evidence.git.abc123"));
    let conn = rusqlite::Connection::open(data_dir.join("companion.sqlite3"))?;
    let payload: String = conn.query_row(
        "SELECT payload_json FROM evidence_events WHERE event_id = 'evidence.git.abc123'",
        [],
        |row| row.get(0),
    )?;
    assert!(payload.contains("implement proposal adapter"));
    assert!(!payload.contains("\"retention\":\"redacted\""));
    Ok(())
}
...
fn retention_enforce_expires_overrides_protects_recovery_and_preserves_privacy(
    ...
    assert_eq!(json["classes"]["raw"]["storeRedacted"], 2);
    assert_eq!(json["classes"]["raw"]["protected"], 1);
...
#[cfg(target_os = "linux")]
#[test]
fn public_scheduler_setup_and_resume_mutate_the_real_user_scheduler_boundary(
) -> Result<(), Box<dyn std::error::Error>> {
    let directory = tempdir()?;
    let home = directory.path().join("home");
    ...
    let systemctl = bash_executable(
        &directory,
        "systemctl",
        &format!(
            "#!/usr/bin/env bash\nprintf '%s\\n' \"$*\" >> '{}'\n",
            directory.path().join("systemctl.log").display()
        ),
    )?;
    std::fs::rename(systemctl, bin.join("systemctl"))?;
    let path = format!(
        "{}:{}",
        bin.display(),
    ...
    assert_eq!(setup["active"], true);
    assert_eq!(setup["scheduler"]["hostSchedulerMutated"], true);
    json_output(
        tracking()?
            .env("HOME", &home)
            .env("PATH", &path)
    ...
    let commands = std::fs::read_to_string(directory.path().join("systemctl.log"))?;
    assert!(commands.contains("enable --now drag-tracking.timer"));
    assert!(commands.contains("disable --now drag-tracking.timer"));
    Ok(())
}
...
fn collect_git_activity_covers_shallow_rewritten_and_unusual_subject_fixtures(
    ...
    assert_eq!(
        outside_date["git"]["commits"].as_array().map(Vec::len),
        Some(0)
    );
    Ok(())
}
...
#[test]
fn gated_execute_counts_terminal_operations_as_skipped_instead_of_blocked(
) -> Result<(), Box<dyn std::error::Error>> {
    let dir = tempdir()?;
    let data_dir = dir.path().join("state");
    let data = data_dir.to_string_lossy();
